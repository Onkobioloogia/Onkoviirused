...
author      : Taavi Päll<br>lektor, Tallinna Tehnikaülikool
output:
...
- viraalsel teel tähendas antud kontekstis, et nad isoleerisid filtreeritava agendi erütro-müeloblastilist leukeemiat põdevate kanade vererakkudest.
- See leid ei tekitanud sellel ajal mingit tähelepanu, lisaks ei peetud tollal erütro-müeloblastilist leukeemiat ka vähiks.
]

.pull-right[
```{r}
knitr::include_graphics("http://www.nature.com/nrc/journal/v10/n12/images/nrc2961-f1.jpg")
```
...
- 1909-11 aastatel katsetas **Francis Peyton Rous** (1879-1970) talle uurimiseks toodud **kana sarkoomiga** ja leidis et tuumorit on võimalik kanadel üle kanda vähirakkudest sõltumatult.


```{r}
knitr::include_graphics("http://o.quizlet.com/wwG.Q0fCHg8IoXzObiRWeg.png")
```


.footer[Pilt: The Biology of Cancer. Garland Science.]
...
```{r, out.height=300}
knitr::include_graphics("http://cancerbio.files.wordpress.com/2011/05/peyton-rous.jpg")
```
...
.pull-left[
```{r}
knitr::include_graphics("http://33.media.tumblr.com/tumblr_m5tnjt5vDR1qbz9meo1_500.jpg")
```
...
```{r}
knitr::include_graphics("http://o.quizlet.com/6TOz6bZnsMrOKfaPM2MXXQ.png")
```
]

.pull-right[
- RSV nakatatud CEF-id moodustavad **kolooniad** (*foci/clusters*)
- kolooniaid moodustavad rakud on muutunud morfoloogiaga (**trasformeerunud**)
- sarnanevad sarkoomi rakkudega
]
...
DNA viirused | | 
B-hepatiidi viirus (HBV) | Hepadnaviridae | 3
SV40/polüoomi viirus | Polyomaviridae | 5
inimese papilloomiviirus 16 (HPV) | Papillomaviridae | 8
inimese adenoviirus 5 | Adenoviridae | 35
inimese herpesviirus 8 (HSV-8; KSHV) | Herpesviridae | 165
Shope fibroomiviirus | Poxviridae | 160
RNA viirused | | 
Rousi sarkoomiviirus (RSV) | Retroviridae | 9
inimese T-raku leukeemia viirus (HTLV-I) | Retroviridae | 9", delim = "|")
virtab <- knitr::kable(vir, format = "html")
kableExtra::kable_styling(virtab, "striped", position = "left", font_size = 12)
```
...
```{r}
knitr::include_graphics("http://imgs.xkcd.com/comics/seven.png")
```


---

# Inimese kasvajaviirused

```{r}
vir <- readr::read_delim("Viirus | Genoom | Kasvaja | Esmakirjeldus 
...
inimese papilloomiviirused (HPV) 16 ja -18 | dsDNA papilloomiviirus | emakakaela vähk, peenisekasvajad, anogenitaal- ja  pea-kaela kasvajad | 1983-84
...
Merkeli rakkude polüoomiviirus | dsDNA polüoomiviirus | Merkeli rakkude kartsinoom | 2008", delim = "|")
virtab <- knitr::kable(vir, format = "html")
kableExtra::kable_styling(virtab, "striped", position = "left", font_size = 12)
```
...
- EBV partiklid avastati elektronmikroskoopia teel nn. **Aafrika Burkitti lümfoomi (BL)** patsientidel.
- EBV on laialt levinud ekvatoriaal Aafrikas ja Paapua Uus-Guineas. 
    - Ainult 2.3% lümfoomidest on BL-d. 
    - ~100% patsientidest nakatunud.
    - võib kaasneda malaariast või kollapalavikust tingitud immuunpuudulikkusega.
- EBV negatiivne **sporaadiline BL** esineb arenenud maades.
    - Sporaadiline BL sisaldab iseloomulikke MYC translokatsioone.


```{r, out.width=260, fig.cap="Pilt: By Ed Uthman, MD. [Public domain], <a href='https://commons.wikimedia.org/wiki/File%3ABurkitt_lymphoma%2C_touch_prep%2C_Wright_stain.jpg'>via Wikimedia Commons</a>"}
knitr::include_graphics("https://upload.wikimedia.org/wikipedia/commons/4/4c/Burkitt_lymphoma%2C_touch_prep%2C_Wright_stain.jpg")
```
...
- HPV-16/18 (all pildil kõige parempoolne HPV16 hübridisatsioon CIN), C hepatiidi, Kaposi sarkoomi ja Merkeli rakkude polüoomiviirus identifitseeriti kasvajates molekulaarbioloogia meetodeid kasutades.

```{r}
knitr::include_graphics("https://www.ncbi.nlm.nih.gov/corecgi/tileshop/tileshop.fcgi?p=PMC3&id=789917&s=6&r=1&c=1")
```
...
```{r, fig.height=3, fig.width=8, dev='svg'}
library(dplyr)
library(purrr)
library(jsonlite)
library(ggplot2)
...
# Anogenitaalsed tüükad (A63.0)
nh04 <- boulder::json_to_df("data/NH04.json")
pk30 <- boulder::json_to_df("data/PK30.json")

mut_vanus <- function(x){
  strsplit(x, split = "-") %>% 
    unlist() %>% 
    sub("\\+", "", .) %>% 
    as.numeric() %>% 
...
nh04filt <- select(nh04, Aasta, Sugu, Vanuserühm, value) %>% 
...
nh04filt <- nh04filt %>% 
  mutate(Vanuserühm = as.character(Vanuserühm),
         Vanuserühm = map_dbl(Vanuserühm, mut_vanus))

pk30filt <- pk30filt %>% 
  mutate(Vanuserühm = as.character(Vanuserühm),
         Vanuserühm = map_dbl(Vanuserühm, mut_vanus))

nh04sum <- group_by(nh04filt, Vanuserühm, Sugu) %>% 
  summarise(Juhud = mean(value, na.rm = T))
pk30sum <- group_by(pk30filt, Vanuserühm, Sugu) %>% 
  summarise(Juhud = mean(value, na.rm = T))

p <- ggplot(nh04sum, aes(Vanuserühm, Juhud, colour = Sugu, group = Sugu)) +
  geom_point() +
  geom_line() +
  ggtitle(unique(nh04$Nakkushaigus),
          paste(range(as.character(nh04$Aasta)), collapse = " - ")) +
  scale_color_colorblind() +
  theme(legend.position = c(0.75,0.75),
        legend.background = element_blank(),
        legend.title = element_blank())

q <- ggplot(pk30sum, aes(Vanuserühm, Juhud, group = 1)) +
  geom_point() +
  geom_line() +
  ggtitle("Emakakaela vähk (C53)",
          paste(range(as.character(pk30$Aasta)), collapse = " - ")) +
  ylab("Intsidents 100 000 inimese kohta") +
  scale_color_colorblind()
...
.footer[
Graafiku andmed: Tervisestatistika ja terviseuuringute andmebaas. [Tervise arengu instituut](http://pxweb.tai.ee/PXWeb2015/index.html).
]

---
# Kaposi sarkoom ja AIDS

Enne AIDSi epideemiat oli Kaposi sarkoom maailmas üliharuldane.

```{r}
knitr::include_graphics("http://1.bp.blogspot.com/-kY9S0A0KJkQ/TY0pqUgWCGI/AAAAAAAAA_M/hyT-Z9YJDhw/ht_philadelphia_051209_ssh.jpg")
```
...
# Rousi sarkoomi viiruse geenide aktiivsus on vajalik transformatsiooniks
...
```{r}
knitr::include_graphics("http://www.nature.com/nrc/journal/v12/n9/images/nrc3320-f1.jpg")
```
...
```{r}
knitr::include_graphics("http://o.quizlet.com/Eby4zdSHLvqEuPGeLks7bA.png")
```
]
...
```{r}
vir <- readr::read_delim("Klass | Näide | Viirus
...
lipiidikinaas | Pi3k | linnu sarkoomiviirus 16", delim = "|")
virtab <- knitr::kable(vir, format = "html")
kableExtra::kable_styling(virtab, "striped", position = "left", font_size = 12)
```
...
```{r}
vir <- readr::read_delim("Geen | Viirus | Kasvaja | Liik | Onkovalk
...
CycD1 | F-MLV | T-raku lümfoom | hiir | G1 tsükliin", delim = "|")
virtab <- knitr::kable(vir, format = "html")
kableExtra::kable_styling(virtab, "striped", position = "left", font_size = 12)
```
...
```{r, out.width=260}
knitr::include_graphics("http://www.nature.com/nrc/journal/v10/n12/images/nrc2961-f3.jpg")
```
...
```{r, out.height=380}
knitr::include_graphics("http://www.nature.com/nature/journal/v506/n7488/images/nature12881-f3.jpg")
```

]

.footer[Pilt: The genomic landscape and evolution of endometrial carcinoma progression and abdominopelvic metastasis. [doi:10.1038/ng.3602](http://www.nature.com/ng/journal/vaop/ncurrent/full/ng.3602.html)
]
...
- Mutations in the HLA-B, HLA-A and in the $\beta2$-microglobulin (B2M) genes. 
...
```{r, out.width=240}
knitr::include_graphics("assets/img/Gibson2016SuppFig12.png")
```
...
```{r}
knitr::include_graphics("assets/img/gastriccancer.jpg")
```
